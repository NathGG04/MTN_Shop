...
namespace NTM_SHOP.Helper
{
    public static class Utilities
    {
        ...
        public static string HashPassword(string password)
        {
            ...
            {
                // Chuyển chuỗi mật khẩu thành mảng bytes
                var bytes = Encoding.UTF8.GetBytes(password);
                // Hash mảng bytes
                var hash = sha256.ComputeHash(bytes);
                // Chuyển hash bytes trở lại thành chuỗi (Base64)
                return Convert.ToBase64String(hash);
            }
        }

        // Hàm xác thực mật khẩu (dùng khi đăng nhập)
        public static bool VerifyPassword(string enteredPassword, string storedHash)
        {
            // Mã hóa mật khẩu người dùng nhập
            string hashOfEnteredPassword = HashPassword(enteredPassword);

            // So sánh với hash đã lưu trong database
